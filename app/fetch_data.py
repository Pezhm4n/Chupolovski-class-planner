...
from app.xml_parser import parse_courses_from_xml
import os
...
class GolestanSession:
    ...
    def authenticate(self, username=None, password=None, max_attempts=3):
        """
        ...
        for i in range(max_attempts):
            ...
            captcha_text = predict_from_path('captcha.png')
            payload = {
                '__VIEWSTATE': results['viewstate'],
                '__VIEWSTATEGENERATOR': results['viewstategen'],
                '__EVENTVALIDATION': results['eventvalidation'],
...
def get_courses(status='both', username=None, password=None):
    ...
    try:
        ...
        if 'available' in results:
            parse_courses_from_xml(results['available'], 'available_courses.json')
            print("💾 Available courses saved to available_courses.json")
        if 'unavailable' in results:
            parse_courses_from_xml(results['unavailable'], 'unavailable_courses.json')
            print("💾 Unavailable courses saved to unavailable_courses.json")
    finally:
